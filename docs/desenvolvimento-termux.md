...
- **Suíte Automatizada Completa**: **11 Quality Gates sequenciais** cobrindo fluxo operacional, isolamento multi-tenant, resiliência offline, ergonomia móvel, RBAC universal e contratos relacionais.
...
flowchart TD
    ...
    M --> TEST["🧪 Bateria Local: 11 Quality Gates\n(npm test && npm run test:mobile)"]
...
# 2. Executar a suíte completa de testes no PWA (11 Quality Gates)
cd PaletScan_pwa
npm test && npm run test:mobile
