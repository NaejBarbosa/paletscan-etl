...
| **WatermelonDB** *(IndexedDB / LokiJS)* | Smartphone / Coletor | **Persistente** *(Banco Local-First)* | • `produtos`: 3.776 SKUs do catálogo mestre higienizado.<br>• `paletes`: Cargas ativas e baixadas na filial.<br>• `codigos_barras`: Variantes de DUN-14 e pesagem. | `< 5ms` | Atualização delta via `syncLocalDB` ou `unsafeResetDatabase()` forçado. |
